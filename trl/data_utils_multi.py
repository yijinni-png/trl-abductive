...
def apply_chat_template(
    ...
    multi_dpo_keys = {"prompt", "chosen_response", "rejected_response", "response", "chosen_prompt", "rejected_prompt"}

    # Add the case for the last possibility
    if example_keys == multi_dpo_keys:
    # Assume the format of the values paired with all keys to be lists of content
    # Assume the prompt is given by the assistant
        output = {}
        output['prompt'] = tokenizer.apply_chat_template(
            example["prompt"],
            tools=tools,
        ...
        output['chosen_response'] = tokenizer.apply_chat_template(
