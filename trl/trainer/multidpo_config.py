...
@dataclass
class MultiDPOConfig(TrainingArguments):
    r"""
    Configuration class for the [`MultiDPOTrainer`].

    This class includes parameters that are specific to MultiDPO training, which combines DPO and ADPO losses.
    ...
    MultiDPO (Multi-objective Direct Preference Optimization) trains with both:
    - DPO loss: comparing chosen vs rejected responses given the same prompt
    - ADPO loss: comparing the same response given chosen vs rejected prompts
    - Combined loss: λ * DPO_loss + (1-λ) * ADPO_loss
    ...
    lambda_weight: float = field(
        default=0.5,
        metadata={
            "help": "Weight parameter for combining DPO and ADPO losses in MultiDPO training. "
            "Final loss = λ * DPO_loss + (1-λ) * ADPO_loss. "
            "λ=1.0 means pure DPO, λ=0.0 means pure ADPO, λ=0.5 means equal weighting. "
    ...
    def __post_init__(self):
        ...
        # MultiDPO-specific validation
        if not (0.0 <= self.lambda_weight <= 1.0):
            raise ValueError(
                f"lambda_weight must be between 0.0 and 1.0, got {self.lambda_weight}. "
                "Use 1.0 for pure DPO, 0.0 for pure ADPO, or values in between for combined training."
            )
        
        # Warn about potentially problematic configurations
        if self.lambda_weight in [0.0, 1.0]:
            import warnings
            mode = "pure ADPO" if self.lambda_weight == 0.0 else "pure DPO"
            warnings.warn(
                f"lambda_weight={self.lambda_weight} enables {mode} training. "
                f"Consider using lambda_weight in range [0.2, 0.8] for true MultiDPO training.",
                UserWarning
            )
        
        # Validate that Liger loss is not used (incompatible with MultiDPO)
        if self.use_liger_loss:
            raise ValueError(
                "Liger loss (use_liger_loss=True) is not compatible with MultiDPO training. "
                "MultiDPO uses a 4-part concatenation structure that requires custom loss computation. "
        ...
        if hasattr(self, 'learning_rate') and self.learning_rate < 1e-7:
            ...
            warnings.warn(
                ...
                f"This might cause parameters to not update properly during MultiDPO training. "
