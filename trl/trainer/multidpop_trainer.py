...
from .multidpop_config import MultiDPOPConfig, FDivergenceConstants, FDivergenceType
from .utils import (
    RunningMoments,
    cap_exp,
...
class MultiDPOPTrainer(Trainer):
    """
    Trainer for Multi-objective Direct Preference Optimization with Penalty (MultiDPOP) method.

    MultiDPOP combines DPOP and ADPOP losses: λ * DPOP_loss + (1-λ) * ADPOP_loss
    where DPOP/ADPOP losses include penalty terms: -F.logsigmoid(β * (logits - λ_dpop * penalty))
    ...
    def __init__(
        ...
        args: Optional[MultiDPOPConfig] = None,
    ...
    def multidpo_loss(
        ...
        """
        # Compute DPOP loss: chosen_response vs rejected_response (given prompt) with penalty
        ...
        # Compute ADPOP loss: response given chosen_prompt vs response given rejected_prompt with penalty
        ...
        # Combine losses using lambda_weight: λ * DPOP_loss + (1-λ) * ADPOP_loss
        ...
        if hasattr(self, 'state') and self.state.global_step % 5 == 0:
            if hasattr(self, 'accelerator') and self.accelerator.is_main_process:
                ...
                print(f"\n🧮 MultiDPOP Loss Debug (Step {self.state.global_step}):")
                print(f"  🔷 DPOP Loss: {dpo_loss_mean:.6f}")
                print(f"  🔶 ADPOP Loss: {adpo_loss_mean:.6f}")
                print(f"  ⚖️  Lambda weight: {lambda_weight:.3f}")
                print(f"  🔸 Combined Loss: {combined_loss_mean:.6f}")
                print(f"  📏 DPOP contribution: {lambda_weight * dpo_loss_mean:.6f}")
                print(f"  📐 ADPOP contribution: {(1-lambda_weight) * adpo_loss_mean:.6f}")
                print(f"  🎯 Lambda DPOP (penalty): {self.lambda_dpop:.3f}")
                
                # Check if ADPOP is getting suppressed
                if adpo_loss_mean > dpo_loss_mean * 10:
                    print("⚠️  ADPOP loss much larger than DPOP - possible scaling issue!")
                elif adpo_loss_mean < dpo_loss_mean / 10:
                    print("⚠️  ADPOP loss much smaller than DPOP - possible underweighting!")
                
                # Debug penalty effects
                if self.lambda_dpop > 0:
                    print(f"  🚨 Penalty enabled with λ_dpop={self.lambda_dpop:.3f}")
                else:
                    print(f"  ⚪ No penalty applied (λ_dpop=0)")
    ...
    def dpo_loss(
        ...
        """
        Compute the DPOP loss for a batch of policy and reference model log probabilities.
        This applies penalty-based modification to the original DPO loss function.

        The penalty term is: max(0, log(π_ref(y_w) / π_θ(y_w))) = F.relu(-chosen_logratios)
        The modified loss is: -F.logsigmoid(β * (original_logits - λ_dpop * penalty_term))
        ...
        Returns:
            A tuple of three tensors: `(losses, chosen_rewards, rejected_rewards)`. The losses tensor contains the DPOP
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            # Calculate original alpha-divergence logits
            original_logits = (cap_exp(rejected_logratios * -alpha_coef) - cap_exp(chosen_logratios * -alpha_coef)) / alpha_coef
            
            # Apply DPOP penalty term: max(0, log(π_ref(y_w) / π_θ(y_w))) = F.relu(-chosen_logratios)
            penalty_term = F.relu(-chosen_logratios)
            
            # Modify logits with penalty: original_logits - λ_dpop * penalty_term
            logits = original_logits - self.lambda_dpop * penalty_term
        else:
            ...
            # Calculate original DPO logits
            original_logits = logratios - ref_logratios
