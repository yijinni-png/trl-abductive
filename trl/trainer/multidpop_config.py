...
@dataclass
class MultiDPOPConfig(TrainingArguments):
    r"""
    Configuration class for the [`MultiDPOPTrainer`].

    This class includes parameters that are specific to MultiDPOP training, which combines DPOP and ADPOP losses.
    MultiDPOP applies a penalty-based modification to both DPO and ADPO loss functions.
    For a full list of training arguments, please refer to the [`~transformers.TrainingArguments`] documentation. 
    Note that default values in this class may differ from those in [`~transformers.TrainingArguments`].

    MultiDPOP (Multi-objective Direct Preference Optimization with Penalty) trains with both:
    - DPOP loss: DPO with penalty term = -F.logsigmoid(β * (dpo_logits - λ_dpop * dpo_penalty))
    - ADPOP loss: ADPO with penalty term = -F.logsigmoid(β * (adpo_logits - λ_dpop * adpo_penalty))
    - Combined loss: λ * DPOP_loss + (1-λ) * ADPOP_loss
    - Penalty term: max(0, log(π_ref(y_w) / π_θ(y_w))) = F.relu(-chosen_logratios)
    ...
    Parameters:
        > MultiDPO Data Requirements
        
        MultiDPO training requires datasets with the following keys:
        ...
        model_init_kwargs (`dict[str, Any]` or `None`, *optional*, defaults to `None`):
            ...
            [`MultiDPOTrainer`] is provided as a string.
    ...
    lambda_weight: float = field(
        default=0.5,
        metadata={
            "help": "Weight parameter for combining DPOP and ADPOP losses in MultiDPOP training. "
            "Final loss = λ * DPOP_loss + (1-λ) * ADPOP_loss. "
            "λ=1.0 means pure DPOP, λ=0.0 means pure ADPOP, λ=0.5 means equal weighting. "
            "Range: [0.0, 1.0]. Recommended: 0.3-0.7 for balanced training."
        },
    )
    lambda_dpop: float = field(
        default=0.0,
        metadata={
            "help": "Penalty coefficient for DPOP modification applied to both DPO and ADPO losses. "
    ...
    enable_dpo_accuracy_tracking: bool = field(
        default=True,
        metadata={
            "help": "Whether to track DPO-specific accuracy metrics (chosen_logps_dpo > rejected_logps_dpo). "
            "Useful for monitoring how well the model learns the DPO objective."
        },
    )
    enable_adpo_accuracy_tracking: bool = field(
        default=True,
        metadata={
            "help": "Whether to track ADPO-specific accuracy metrics (chosen_logps_adpo > rejected_logps_adpo). "
            "Useful for monitoring how well the model learns the ADPO objective."
        },
    )
    multidpo_debug_mode: bool = field(
        default=False,
        metadata={
            "help": "Enable detailed debugging output for MultiDPO training including parameter norms, "
            "gradient information, and loss component breakdowns. Useful for troubleshooting training issues."
        },
    )
    ...
    def __post_init__(self):
        ...
        if not (0.0 <= self.lambda_weight <= 1.0):
            raise ValueError(
                f"lambda_weight must be between 0.0 and 1.0, got {self.lambda_weight}. "
        ...
        if self.lambda_weight in [0.0, 1.0]:
            import warnings
            mode = "pure ADPOP" if self.lambda_weight == 0.0 else "pure DPOP"
            warnings.warn(
                f"lambda_weight={self.lambda_weight} enables {mode} training. "
                f"Consider using lambda_weight in range [0.2, 0.8] for true MultiDPOP training.",
                UserWarning
            )
        
        # Warn about lambda_dpop usage
        if self.lambda_dpop == 0.0:
            import warnings
            warnings.warn(
        ...
        # Validate that Liger loss is not used (incompatible with MultiDPOP)
        if self.use_liger_loss:
            raise ValueError(
                "Liger loss (use_liger_loss=True) is not compatible with MultiDPOP training. "
                "MultiDPOP uses a 4-part concatenation structure that requires custom loss computation. "
                "Please set use_liger_loss=False."
            )
        
        # Warning for very small learning rates that might cause training issues
        if hasattr(self, 'learning_rate') and self.learning_rate < 1e-7:
            import warnings
            warnings.warn(
                f"Very small learning_rate={self.learning_rate} detected. "
                f"This might cause parameters to not update properly during MultiDPOP training. "
                f"Consider using learning rates in range [1e-6, 5e-6] for stable training.",
                UserWarning
            )
